...
# データの読み込み
path = "./titanic_other_model/fixed_data/"
...
# X_prediction = torch.tensor(X_pred.values, dtype=torch.float32)

# # inputをx_train, targetをy_trainとしてdatasetを作成
# train_dataset = TensorDataset(X_train, y_train)
# test_dataset = TensorDataset(X_test, y_test)
# pred_dataset = TensorDataset(X_prediction, torch.zeros(X_pred.shape[0]))
...
def find_best_model(
    ...
    lgbm_acc = round(metrics.accuracy_score(y_test, y_pred) * 100, 2)  # type: ignore
    ...
    print(
        model_df.sort_values("Score", ascending=False).reset_index(drop=True)
    )
...
rfc = RandomForestClassifier(
    random_state=42, n_jobs=-1  # Use all cores on your machine
)
param_grid = {
    "n_estimators": [
        100,
...
CV_rfc = GridSearchCV(
    estimator=rfc, param_grid=param_grid, cv=32, scoring="accuracy", verbose=10
)
CV_rfc.fit(X, Y)
print("")
...
output = pd.DataFrame(
    {"PassengerId": X_pred.PassengerId, "Survived": predictions}
)
...
#         with open("./titanic_other_model/params/hyperparameter.txt", "w") as f:
#             f.write(
...
#     color="r",
# )
# plt.savefig("./titanic_other_model/graphs/accuracy_image.png")
# plt.show()
...
# # 予測結果をcsvファイルに保存
# y_pred.to_csv("./titanic_other_model/predictions/submission.csv", index=False)  # type: ignore
