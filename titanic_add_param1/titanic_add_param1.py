...
data["Age"] = imputer.fit_transform(
    np.hstack((data[features], np.expand_dims(data["Age"], axis=1)))
)[:, -1]
...
data = pd.get_dummies(
    data=data, columns=["Title", "Pclass", "Family_survival"]
)
...
features = [
    ...
    if col.startswith(("Pclass_", "Title_", "Family_survival_"))
] + ["Sex", "Embarked", "SibSp", "Parch", "Ticket", "Family_size"]
