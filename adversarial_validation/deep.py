...
for epoch in range(EPOCH):
    ...
    if earlystopping.early_stop:
        ...
        with open(
            "./adversarial_validation/params/hyperparameter.txt", "w"
        ) as f:
            f.write(
...
# ハイパーパラメータをtxtファイルに保存
with open("./adversarial_validation/params/hyperparameter.txt", "w") as f:
    f.write(
        f"batch_size: {BATCH_SIZE}\n"
        f"weight_decay: {WEIGHT_DECAY}\n"
        f"learning_rate: {LEARNING_RATE}\n"
...
plt.savefig("./adversarial_validation/graphs/accuracy_image.png")
plt.show()
...
# 予測結果をcsvファイルに保存
y_pred.to_csv("./adversarial_validation/predictions/submission.csv", index=False)  # type: ignore
